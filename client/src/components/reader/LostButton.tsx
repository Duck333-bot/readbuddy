...
export function LostButton({ onClick }: { onClick: () => void }) {
  return <div className="fixed bottom-6 right-4 z-40 sm:right-6"><Tooltip><TooltipTrigger asChild><button onClick={onClick} className="flex h-10 w-10 items-center justify-center rounded-full border border-[#8a85c9]/20 bg-card/90 text-muted-foreground shadow-md backdrop-blur transition-all hover:border-[#8a85c9]/55 hover:text-[#6f6ab2] active:scale-95" aria-label="I’m lost — help me understand where I am"><HelpCircle className="h-4 w-4" /></button></TooltipTrigger><TooltipContent side="left">I’m lost</TooltipContent></Tooltip></div>;
}

export function LostReaderCard({ answer, isLoading, onClose }: { answer: string; isLoading: boolean; onClose: () => void }) {
  return <motion.aside initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="mb-8 rounded-2xl border border-[#8a85c9]/20 bg-card/90 shadow-[0_12px_35px_rgba(51,46,83,0.12)] backdrop-blur-sm"><div className="flex items-center justify-between border-b border-[#8a85c9]/12 px-4 py-3"><div className="flex items-center gap-2 text-sm font-semibold text-[#6f6ab2]"><Sparkles className="h-3.5 w-3.5" /> Here’s what matters right now</div><button onClick={onClose} aria-label="Close recap" className="rounded-md p-1 text-muted-foreground hover:bg-accent"><X className="h-3.5 w-3.5" /></button></div><div className="px-4 py-4">{isLoading ? <div className="space-y-2"><Skeleton className="h-3.5 w-full" /><Skeleton className="h-3.5 w-10/12" /><Skeleton className="h-3.5 w-4/5" /></div> : <div className="prose prose-sm max-w-none text-sm leading-relaxed"><Streamdown>{answer}</Streamdown></div>}</div></motion.aside>;
}
